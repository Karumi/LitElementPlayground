...
function resolveNodeValue(node, context) {
    ...
    if (ts.isVariableDeclaration(node)) {
        return resolveNodeValue(node.initializer, __assign({}, context, { depth: depth }));
    }
    // Resolve value of a property access expression. For example: MyEnum.RED
    else if (ts.isPropertyAccessExpression(node)) {
        return resolveNodeValue(node.name, __assign({}, context, { depth: depth }));
    }
    ...
    // Resolve values of variables.
        ...
        return resolveNodeValue(declaration[0], __assign({}, context, { depth: depth }));
    }
    // Fallthrough
    //  - "my-value" as string
    //  - <any>"my-value"
    //  - ("my-value")
        ...
        return resolveNodeValue(node.expression, __assign({}, context, { depth: depth }));
    }
    // static get is() {
    //    return "my-element";
    // }
        ...
        try {
            for (var _c = __values(node.body.statements), _d = _c.next(); !_d.done; _d = _c.next()) {
                ...
                if (ts.isReturnStatement(stm)) {
                    return resolveNodeValue(stm.expression, __assign({}, context, { depth: depth }));
                }
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
...
function relaxType(type) {
    switch (type.kind) {
        ...
        case tsSimpleType.SimpleTypeKind.UNION:
            return __assign({}, type, { types: type.types.map(function (t) { return relaxType(t); }) });
        case tsSimpleType.SimpleTypeKind.ENUM:
            return __assign({}, type, { types: type.types.map(function (t) { return relaxType(t); }) });
        case tsSimpleType.SimpleTypeKind.ARRAY:
            return __assign({}, type, { type: relaxType(type.type) });
        ...
        case tsSimpleType.SimpleTypeKind.ALIAS:
            return __assign({}, type, { target: relaxType(type.target) });
        default:
            return type;
    }
}
...
function parseLitPropertyOption(_a, context) {
    ...
    switch (kind) {
        case "converter": {
            return __assign({}, config, { hasConverter: true });
        }
        case "attribute": {
            ...
            return __assign({}, config, { attribute: attribute, node: __assign({}, (config.node || {}), { attribute: initializer }) });
        }
        case "type": {
            var type = void 0;
            ...
            return __assign({}, config, { type: type, node: __assign({}, (config.node || {}), { type: initializer }) });
        }
        // Polymer specific field
        case "value": {
            return __assign({}, config, { default: resolveNodeValue(initializer, { ts: ts, checker: checker }) });
        }
    }
    return config;
}
...
function mergeJsDocs(mainJsDoc, inheritedJsDocs) {
    var jsDoc = inheritedJsDocs.reduce(function (jsDoc, extendJsDoc) {
        if (extendJsDoc == null)
            return jsDoc;
        return __assign({}, jsDoc, { node: jsDoc.node, tags: __spread((jsDoc.tags || []), (extendJsDoc.tags || [])) });
    }, mainJsDoc || {});
...
function mergeDeclarations(declarations, context) {
    ...
    return __assign({}, declarations[0], { jsDoc: mergedJsDoc, members: mergedMembers, slots: mergedSlots, events: mergedEvents, cssProperties: mergedCSSProps });
}
/**
 * Merges slots
 * @param slots
    ...
    try {
        for (var entries_1 = __values(entries), entries_1_1 = entries_1.next(); !entries_1_1.done; entries_1_1 = entries_1.next()) {
            ...
            if (existing == null) {
                ...
                merged.set(entry.name || "", __assign({}, existing, { jsDoc: entry.jsDoc }));
            }
        }
    }
    catch (e_1_1) { e_1 = { error: e_1_1 }; }
...
function mergeAttrIntoProp(prop, attr, checker) {
    return __assign({}, prop, { type: mergeTypes(prop.type, attr.type, checker), default: attr.default || prop.default, required: attr.required || prop.required, jsDoc: attr.jsDoc || prop.jsDoc, attrName: attr.attrName });
}
/**
...
function mergeMemberIntoMember(a, b, checker) {
    return __assign({}, b, { attrName: a.attrName || b.attrName, type: mergeTypes(a.type, b.type, checker) });
}
/**
...
function parseComponentDeclaration(declarationNode, flavors, context) {
    ...
    visitComponentDeclaration(declarationNode, flavors, __assign({}, context, { declarationNode: declarationNode, features: {
...
function executeFirstFlavor(flavors, key, node, context) {
    ...
    var _loop_1 = function (flavor) {
        ...
        var result = func(node, __assign({}, context, { emitContinue: function () {
                shouldContinue = true;
            } }));
...
function parseComponentDefinitionResults(node, flavors, context) {
    ...
    if (node == null)
        return [];
    var resultMap = [];
    var definitionContext = __assign({}, context, { emitDefinitionResult: function (result) {
    ...
    try {
        for (var flavors_1 = __values(flavors), flavors_1_1 = flavors_1.next(); !flavors_1_1.done; flavors_1_1 = flavors_1.next()) {
            ...
            if (flavor.visitGlobalEvents == null)
                continue;
            flavor.visitGlobalEvents(node, __assign({}, context, { emitEvent: function (event) {
    ...
    return results.map(function (result) {
        var tags = result.componentDefinitions.map(function (_a) {
            ...
            return ({
                ...
                members: declaration.members.map(function (res) { return (__assign({}, res, { type: tsSimpleType.toTypeString(res.type, checker), node: null })); }),
                ...
                case 0:
                    ...
                    return [4 /*yield*/, Promise.all(globs.map(function (g) {
                            try {
                                ...
                                if (dirExists) {
                                    return fastGlob.async(__spread((config.analyzeLibraries || g.includes("node_modules") ? [] : IGNORE_GLOBS), [path.join(g, DEFAULT_DIR_GLOB)]), {
                                        absolute: true,
                            ...
                            return fastGlob.async(__spread((config.analyzeLibraries || g.includes("node_modules") ? [] : IGNORE_GLOBS), [g]), {
                                absolute: true,
                                followSymlinkedDirectories: false
                            });
                        }))];
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        // Analyze all globs
                        return [4 /*yield*/, analyzeGlobs(inputGlobs, config, __assign({}, context, { emitAnalyzedFile: function (file, result, _a) {
                    ...
                    case 3:
                        ...
                        fs.writeFileSync(path$1, transformed);
                        return [3 /*break*/, 6];
                    case 4: return [4 /*yield*/, analyzeGlobs(inputGlobs, config, __assign({}, context, { emitAnalyzedFile: function (file, result, _a) {
                                var program = _a.program;
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        config.analyze = __assign({}, (config.analyze || {}), { diagnostics: true });
